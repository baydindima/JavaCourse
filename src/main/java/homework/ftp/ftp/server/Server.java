 ...
 * @param <QueryReader>    query reader type
 * @param <ResponseWriter> response writer type
 */
@Slf4j
public abstract class Server<T,
        QueryReader extends ObjectReader<T>,
        ResponseWriter extends ObjectWriter> implements Closeable {
    /**
     * Default buffer size.
     */
    ...
    public final void start() throws IOException {
        if (isStarted) {
            throw new InvalidProcessorStateException("Server already started!");
        }
        if (isClosed) {
            throw new InvalidProcessorStateException("Server already closed!");
        }
        isStarted = true;
    ...
    private void readQuery(@NotNull final AsynchronousSocketChannel client) {
        ...
        QueryReader queryReader = getReaderSupplier().get();
     ...
     */
    @NotNull
    protected abstract Supplier<@NotNull QueryReader>
    getReaderSupplier();
    ...
    defaultWrite(@NotNull final ResponseWriter writer) {
        return client -> {
